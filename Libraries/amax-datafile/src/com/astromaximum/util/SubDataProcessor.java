...
public abstract class SubDataProcessor {
    ...
    protected int read(DataInputStream is, int evtype, int planet,
        ...
        try {
            ...
            int cumul;
            long date = 0;
            //System.out.println("totalCount = " + totalCount + ", dayStart = " + dayStart + ", dayEnd = " + dayEnd);
            for (int i = 0; i < totalCount; i++) {
                ...
                if (last.isInPeriod(dayStart, dayEnd, false)) {
                    //System.out.println("addEvent1 #" + eventsCount + ": " + last.toString());
                    addEvent(eventsCount, last);
                    eventsCount++;
                } else {
                    //System.out.println("not in period: " + last.toString());
                    if (eventsCount > 0) {
                        break;
                    }
                }
            ...
            if (last.isInPeriod(dayStart, dayEnd, false)) {
                //System.out.println("addEvent2 #" + eventsCount + ": " + last.toString());
                addEvent(eventsCount, last);
                eventsCount++;
            }
